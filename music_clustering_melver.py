...
# mp3の読み込み
music,fs = librosa.load('music/Session High↑.mp3')
...
# 短時間フーリエ変換
S = librosa.feature.melspectrogram(y=music, sr=fs, n_mels=128, fmax=8000)

# 結果をスペクトログラムで表示
librosa.display.specshow(librosa.power_to_db(S,ref=np.max),
                         y_axis='mel',x_axis='time',hop_length=hop_length,fmax=8000)
...
# クラスタ分類(k_means)
logamp = librosa.power_to_db(S,ref=np.max)
