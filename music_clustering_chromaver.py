...
# クロマグラム作成
D = librosa.feature.chroma_cens(y=y_harmonic,sr=sr)
...
# クラスタ分類(k_means)
logamp = librosa.power_to_db(D,ref=np.max)
...
# index内容
index = [(len(y)/sr)/len(count_list)*x for x in range(len(count_list))] # 秒数
